...
NOMBRE_PROGRAMA = "h264_enc"  # Cambiado a h264_enc
...
def ejecutar_simulacion(config, simulacion):
    ...
    # Llama al script de gem5 con los parámetros adecuados para h264_enc
    command = [
    ...
    archivo_yuv = os.path.join(carpeta_m5out, "h264dec_outfile.yuv")
    archivo_estadisticas_gem5 = os.path.join(carpeta_m5out, "stats.txt")

    if os.path.exists(archivo_yuv):
        print(f"Archivo de salida encontrado: {archivo_yuv}")
    else:
